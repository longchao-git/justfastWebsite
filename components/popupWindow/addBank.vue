			...
			<div>
				...
				<div class='loginClass'>
					<div class='login_input'>
						...
						<div class='' style='display: flex;align-items: center'>
							<img @click='setcard_type(1)' v-if='card_type == 1' src='../../assets/images/icon_get1.jpg'
								style='width: 40px; height: 22px;' alt='' />
							...
							<img @click='setcard_type(2)' v-if='card_type == 2' src='../../assets/images/icon_get2.jpg'
								style='width: 40px; height: 22px;margin-left: 10px' alt='' />
					...
					<div class='login_input'>
						...
						<input v-model='card_name' :placeholder="$t('addAddr.ingrese')" class='c-input' />
					</div>
					...
					<v-btn width='100%' height='48px' class='try-out-bt mt3'
						@click='createPaymentMethod()'>{{ $t(`bank.but`) }}
					</v-btn>
				</div>
			</div>
		</div>
				...
				this.$axios.post('/client/member/card/setup_intent', params).then(res => {
					this.stripe.confirmCardSetup(res.client_secret, {
						...
						if (result.setupIntent && result.setupIntent.payment_method) {
							...
							this.loading = false
							this.$message.info(result.error && result.error.message ? result.error
								.message : this.$t(`failtoadd`));
						}
					}).catch(err => {
						this.loading = false
						this.$message.info(err.message);
