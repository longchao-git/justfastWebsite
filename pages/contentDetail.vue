<template>
  <div class='detail_container' :class='loginType!=-1?"isview_container":""'>
    <el-image class='fit-cover' fit='cover' :src='topInfo.shop_show?topInfo.shop_show:topInfo.logo'
      ...
      <div class='' style='display: flex;flex-direction: row;align-items: center;justify-content: space-between'>
        <div class=' flex-column viewViewCkass' style='margin-top: 50px; display: flex;margin-left: 30px'>
          ...
          <div style='align-items: center;display: flex;'>
            <div class='text_amount' style='font-size: 14px;color: #EE8080'>
              {{ $t('home.partir') }}€{{ topInfo.min_amount }}
            </div>
            <div class='text_freight ' style='font-size: 14px;margin-left: 10px;color: #EE8080'>
              <span v-if='topInfo.freight == 0'>{{ $t('creation.gastos') }}</span>
              <span v-else-if='topInfo.is_reduce_pei == 1'>{{ $t('home.postageandpackingfee')
                }}€{{ topInfo.reduceEd_freight }}</span>
              <span v-else>{{ $t('home.postageandpackingfee') }}€{{ topInfo.freight }}</span>
            </div>
          </div>
        </div>
        <div class='flex flex-a-c butoonView' style='cursor: pointer;justify-content: flex-end;margin: 0 auto;'>
          ...
          <span style='color: #ee8080;margin-left: 10px'>{{ $t(`importetotal`) }}：€{{ (min_amount).toFixed(2)
            }}</span>
          <div @click='handleCloseLoginDialog(1)' class='button_info' style='margin-left: 32px'>
            {{ $t('contentDetail.name')
